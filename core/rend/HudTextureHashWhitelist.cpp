...
HudTextureHashWhitelist::HudTextureHashWhitelist() : m_whitelistedHashes() {
  // Hashes are moved into the unordered_set
}

void HudTextureHashWhitelist::populateFromDefaultYamlFile(
    HudTextureHashWhitelist &ctx) {

  std::string game_id = library::getGameId();
  if (!game_id.empty()) {
    return populateFromYamlFile(ctx, hostfs::getHudConfigurationPath() +
                                         game_id + "/hud.yaml");
  } else {
    ERROR_LOG(RENDERER,
              "No game ID loaded, using default HUD texture whitelist");
    ctx.init(std::vector<u32>{});
  }
}

// Factory method to create an instance from a YAML file
void HudTextureHashWhitelist::populateFromYamlFile(
    HudTextureHashWhitelist &ctx, const std::string &file_path) {
  NOTICE_LOG(RENDERER, "Loading HUD texture whitelist from: %s",
             file_path.c_str());
  ...
  if (!file.is_open()) {
    WARN_LOG(RENDERER, "Failed to open HUD texture whitelist file: %s",
             file_path.c_str()); // Warn log added
    ctx.init(std::vector<u32>{});
    return;
  }
  ...
  while (std::getline(file, line)) {
    ...
    if (in_texture_hashes_section) {
      if (trimmed_line.rfind("- ", 0) == 0) { // Starts with "- "
        std::string hash_str =
            trim(trimmed_line.substr(2)); // Remove "- " prefix
        // Remove quotes if present
        if (hash_str.length() >= 2 && hash_str.front() == '"' &&
            hash_str.back() == '"') {
        ...
        if (!hash_str.empty()) {
          ...
          auto [ptr, ec] = std::from_chars(hash_str.data(),
                                           hash_str.data() + hash_str.length(),
                                           hash_val, 16);

          if (ec == std::errc()) {
            NOTICE_LOG(RENDERER, "Adding texture hash to hud whitelist: 0x%X",
                       hash_val);
            hashes.push_back(hash_val);
          } else {
            WARN_LOG(RENDERER,
                     "Failed to convert hash string '%s' to u32. Error: %d",
                     hash_str.c_str(), static_cast<int>(ec));
          }
        }
