...
static SDL_Rect windowPos{SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED,
...
static void sdl_open_joystick(int index) {
  ...
  try {
#ifdef __SWITCH__
    std::shared_ptr<SDLGamepad> gamepad = std::make_shared<SwitchGamepad>(
        index < MAPLE_PORTS ? index : -1, index, pJoystick);
#elif defined(USE_DREAMLINK_DEVICES)
    std::shared_ptr<SDLGamepad> gamepad;
    if (DreamLinkGamepad::isDreamcastController(index))
      gamepad = createDreamLinkGamepad(index < MAPLE_PORTS ? index : -1, index,
                                       pJoystick);
    else
      gamepad = std::make_shared<SDLGamepad>(index < MAPLE_PORTS ? index : -1,
                                             index, pJoystick);
#else
    std::shared_ptr<SDLGamepad> gamepad = std::make_shared<SDLGamepad>(
        index < MAPLE_PORTS ? index : -1, index, pJoystick);
#endif
...
static void setWindowTitleGame() {
  if (settings.naomi.slave)
    SDL_SetWindowTitle(window, ("Flycast - Multiboard Slave " +
                                config::loadStr("naomi", "BoardId"))
                                   .c_str());
  else
...
static void captureMouse(bool capture) {
  ...
  if (!capture) {
    ...
    if (config::UseRawInput || SDL_SetRelativeMouseMode(SDL_TRUE) == 0) {
      if (config::UseRawInput)
...
void input_sdl_init() {
  ...
  // Event::Start is called on a background thread, so we can't use it to change
  // the window title (macOS) However it's followed by Event::Resume which is
  // fine.
  ...
  // This is used for both on-screen and regular keyboards. For the latter, it
  // disables text input processing when not required, which fixes the accent
  // menu showing up on macOS and may improve performance on all platforms.
#ifndef __SWITCH__
  ...
  // Can be removed once mapping is merged into SDL, see
  // https://github.com/libsdl-org/SDL/pull/12039
#if (defined(__APPLE__) && defined(TARGET_OS_MAC))
  SDL_GameControllerAddMapping(
      "0300000009120000072f000000010000,OrangeFox86 "
      "DreamPicoPort,a:b0,b:b1,x:b3,y:b4,dpleft:h0.8,dpright:h0.2,dpup:h0.1,"
      "dpdown:h0.4,leftx:a0,lefty:a1,lefttrigger:a2,rightx:a3,righty:a4,"
      "righttrigger:a5,start:b11");
#elif defined(_WIN32)
  SDL_GameControllerAddMapping(
      "0300000009120000072f000000000000,OrangeFox86 "
      "DreamPicoPort,a:b0,b:b1,x:b3,y:b4,dpleft:h0.8,dpright:h0.2,dpup:h0.1,"
      "dpdown:h0.4,leftx:a0,lefty:a1,lefttrigger:a2,rightx:a3,righty:a4,"
      "righttrigger:a5,start:b11");
#endif
...
void input_sdl_handle() {
  ...
  while (SDL_PollEvent(&event)) {
      ...
      if (event.key.repeat == 0) {
        auto is_key_mapped = [](u32 code) -> bool {
          const InputMapping::InputSet inputSet{
              InputMapping::InputDef::from_button(code)};
#if defined(_WIN32) && !defined(TARGET_UWP)
          if (config::UseRawInput) {
            for (int i = 0; i < GamepadDevice::GetGamepadCount(); i++) {
              auto gamepad = GamepadDevice::GetGamepad(i);
              if (dynamic_cast<rawinput::RawKeyboard *>(gamepad.get()) !=
                  nullptr) {
                bool mapped = (gamepad->get_input_mapping()->get_button_id(
                                   0, inputSet) != EMU_BTN_NONE);
                if (mapped)
                  return true;
              }
            }
...
#endif
          {
            return (sdl_keyboard->get_input_mapping()->get_button_id(
                        0, inputSet) != EMU_BTN_NONE);
          }
        };
        if (event.type == SDL_KEYDOWN) {
          ...
          if ((event.key.keysym.sym == SDLK_RETURN &&
              ...
              (event.key.keysym.sym == SDLK_F11 &&
               (event.key.keysym.mod &
                (KMOD_ALT | KMOD_CTRL | KMOD_SHIFT | KMOD_GUI)) == 0)) {
          ...
          if ((event.key.keysym.mod & KMOD_LALT) &&
              ...
              !(is_key_mapped(SDL_SCANCODE_LALT) ||
                is_key_mapped(SDL_SCANCODE_LCTRL))) {
            captureMouse(!mouseCaptured);
            break;
          }
          // Barcode scanner
        ...
        if (!config::UseRawInput)
          sdl_keyboard->input(event.key.keysym.scancode,
    ...
    case SDL_WINDOWEVENT:
      if (event.window.event == SDL_WINDOWEVENT_SIZE_CHANGED ||
        ...
        if (windowFlags & SDL_WINDOW_VULKAN)
          SDL_Vulkan_GetDrawableSize(window, &settings.display.width,
                                     &settings.display.height);
        else
#endif
#ifdef USE_OPENGL
            if (windowFlags & SDL_WINDOW_OPENGL)
          SDL_GL_GetDrawableSize(window, &settings.display.width,
                                 &settings.display.height);
        else
#endif
          SDL_GetWindowSize(window, &settings.display.width,
                            &settings.display.height);
    ...
    case SDL_JOYBUTTONUP: {
      std::shared_ptr<SDLGamepad> device =
          SDLGamepad::GetSDLGamepad((SDL_JoystickID)event.jbutton.which);
      if (device != NULL)
        device->gamepad_btn_input(event.jbutton.button,
                                  event.type == SDL_JOYBUTTONDOWN);
    } break;
    case SDL_JOYAXISMOTION: {
      std::shared_ptr<SDLGamepad> device =
          SDLGamepad::GetSDLGamepad((SDL_JoystickID)event.jaxis.which);
      if (device != NULL)
        device->gamepad_axis_input(event.jaxis.axis, event.jaxis.value);
    } break;
    case SDL_JOYHATMOTION: {
      std::shared_ptr<SDLGamepad> device =
          SDLGamepad::GetSDLGamepad((SDL_JoystickID)event.jhat.which);
      if (device != NULL) {
    ...
    case SDL_MOUSEMOTION:
      ...
      if (!config::UseRawInput) {
        ...
        x /= 2;
        y /= 2;
        if (std::abs(x - event.motion.x) > 10 ||
            std::abs(y - event.motion.y) > 10)
...
bool sdl_recreate_window(u32 flags) {
  ...
  if (shcoreDLL) {
    ...
    if (SetProcessDpiAwareness) {
      ...
      if (SDL_GetDisplayDPI(0, &settings.display.dpi, NULL, NULL) !=
  ...
  window_fullscreen =
      config::loadBool("window", "fullscreen", window_fullscreen);
  window_maximized = config::loadBool("window", "maximized", window_maximized);
  if (window != nullptr)
    get_window_state();

  // Check if the saved window position is on a valid display, preventing
  // Flycast from opening on a screen no longer pluged in
  ...
  if (numDisplays > 0) {
    ...
    // If position is invalid, reset to primary display, avoiding Flycast from
    // opening in a missing window and not being seen when windowed
    if (!validPosition) {
      NOTICE_LOG(COMMON, "Saved window position is not on any connected "
                         "display, resetting to primary display");
  ...
  if (flags & SDL_WINDOW_RESIZABLE) {
    // The position passed to SDL_CreateWindow doesn't take decorations into
    // account on linux. SDL_ShowWindow retrieves the border dimensions and
    // SDL_SetWindowPosition uses them to correctly (re)position the window if
    // needed.
  ...
  SDL_Surface *surface =
      SDL_CreateRGBSurfaceFrom(pixels, 48, 48, 32, 4 * 48, 0x00ff0000,
  ...
  if (displayIndex < 0)
    WARN_LOG(RENDERER, "Cannot get the window display index: %s",
             SDL_GetError());
  else {
    SDL_DisplayMode mode{};
    if (SDL_GetDesktopDisplayMode(displayIndex, &mode) == 0) {
      NOTICE_LOG(RENDERER, "Monitor refresh rate: %d Hz (%d x %d)",
...
void sdl_window_create() {
  ...
  try {
    ...
    SDL_ShowSimpleMessageBox(SDL_MESSAGEBOX_ERROR, i18n::T("Flycast Error"),
                             e.what(), nullptr);
    throw;
  }
  // ImGui copy & paste
...
// and use '*' as preamble and terminator, which are the Code 39 start and stop
// characters. So disable the default terminator ('\n') and enable sending the
// Code 39 start and stop characters.
static bool handleBarcodeScanner(const SDL_Event &event) {
  static const std::unordered_map<u16, char> keymapDefault{
      {SDL_SCANCODE_SPACE, ' '},     {0x100 | SDL_SCANCODE_B, 'B'},
      {0x100 | SDL_SCANCODE_C, 'C'}, {0x100 | SDL_SCANCODE_D, 'D'},
      {0x100 | SDL_SCANCODE_E, 'E'}, {0x100 | SDL_SCANCODE_F, 'F'},
      {0x100 | SDL_SCANCODE_G, 'G'}, {0x100 | SDL_SCANCODE_H, 'H'},
      {0x100 | SDL_SCANCODE_I, 'I'}, {0x100 | SDL_SCANCODE_J, 'J'},
      {0x100 | SDL_SCANCODE_K, 'K'}, {0x100 | SDL_SCANCODE_L, 'L'},
      {0x100 | SDL_SCANCODE_N, 'N'}, {0x100 | SDL_SCANCODE_O, 'O'},
      {0x100 | SDL_SCANCODE_P, 'P'}, {0x100 | SDL_SCANCODE_R, 'R'},
      {0x100 | SDL_SCANCODE_S, 'S'}, {0x100 | SDL_SCANCODE_T, 'T'},
      {0x100 | SDL_SCANCODE_U, 'U'}, {0x100 | SDL_SCANCODE_V, 'V'},
      {0x100 | SDL_SCANCODE_X, 'X'},
  };
  static const std::unordered_map<u16, char> keymapUS{
      {0x100 | SDL_SCANCODE_8, '*'}, {SDL_SCANCODE_MINUS, '-'},
      {SDL_SCANCODE_PERIOD, '.'},    {0x100 | SDL_SCANCODE_4, '$'},
      {SDL_SCANCODE_SLASH, '/'},     {0x100 | SDL_SCANCODE_EQUALS, '+'},
      {0x100 | SDL_SCANCODE_5, '%'}, {0x100 | SDL_SCANCODE_A, 'A'},
      {0x100 | SDL_SCANCODE_M, 'M'}, {0x100 | SDL_SCANCODE_Q, 'Q'},
      {0x100 | SDL_SCANCODE_W, 'W'}, {0x100 | SDL_SCANCODE_Y, 'Y'},
      {0x100 | SDL_SCANCODE_Z, 'Z'}, {SDL_SCANCODE_0, '0'},
  ...
  static const std::unordered_map<u16, char> keymapCH{
      {0x100 | SDL_SCANCODE_3, '*'}, {SDL_SCANCODE_SLASH, '-'},
      {SDL_SCANCODE_PERIOD, '.'},    {SDL_SCANCODE_BACKSLASH, '$'},
      {0x100 | SDL_SCANCODE_7, '/'}, {0x100 | SDL_SCANCODE_1, '+'},
      {0x100 | SDL_SCANCODE_5, '%'}, {0x100 | SDL_SCANCODE_A, 'A'},
      {0x100 | SDL_SCANCODE_M, 'M'}, {0x100 | SDL_SCANCODE_Q, 'Q'},
      {0x100 | SDL_SCANCODE_W, 'W'}, {0x100 | SDL_SCANCODE_Y, 'Z'},
      {0x100 | SDL_SCANCODE_Z, 'Y'}, {SDL_SCANCODE_0, '0'},
      {SDL_SCANCODE_1, '1'},         {SDL_SCANCODE_2, '2'},
      {SDL_SCANCODE_3, '3'},         {SDL_SCANCODE_4, '4'},
      {SDL_SCANCODE_5, '5'},         {SDL_SCANCODE_6, '6'},
      {SDL_SCANCODE_7, '7'},         {SDL_SCANCODE_8, '8'},
  ...
  if (it == keymap->end()) {
    ...
    if (it == keymapDefault.end()) {
      if (!barcode.empty()) {
        INFO_LOG(INPUT, "Unrecognized barcode scancode %d mod 0x%x", scancode,
                 mod);
        barcode.clear();
      }
      return false;
...
void pauseHaptic() { SDLGamepad::SetTorque(0, 0.f); }

void resumeHaptic() { SDLGamepad::SetTorque(0, torque); }
